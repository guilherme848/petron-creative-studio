...
export default function EditarClientePage() {
  ...
  useEffect(() => {
    async function fetchClient() {
      try {
        ...
        const client = data.find((c: { id: string }) => c.id === params.id);
        ...
        setForm({
          ...
          logoFile: null,
          cores: brand?.colors?.length > 0 ? brand.colors : [...DEFAULT_COLORS],
          fonteTitulo: fonts.title || "",
          fontePreco: fonts.price || "",
          fonteDescricao: fonts.description || "",
        });
      } catch {
  ...
  const removeLogo = () => {
    if (form.logoUrl && form.logoFile) URL.revokeObjectURL(form.logoUrl);
    setForm((prev) => ({
      ...prev,
      logoUrl: null,
      logoFile: null,
